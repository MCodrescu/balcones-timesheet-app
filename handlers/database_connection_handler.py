...
class DatabaseConnectionHandler:
    ...
    def get_all_jobs(self, cache_seconds = None):
        ...
        result = self.conn.query(
            f"SELECT job_number FROM job_register.jobs \
            LEFT JOIN job_register.employee_time USING (job_number) \
            GROUP BY job_number \
            ORDER BY SUM(hours_worked) / (CURRENT_DATE - MIN(work_date)) DESC",
            ttl = cache_seconds
        )
